...
export default function About() {
  ...
  return (
    <div className="min-h-screen pt-16 lg:pt-20">
      ...
      {/* Stats Section */}
      <section className="section bg-white dark:bg-gray-900 -mt-16 relative z-10">
        <div className="container">
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl p-8 lg:p-12">
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-8">
              {stats.map((stat, index) => (
                <Parallax key={index} speed={index * 2}>
                  <div className="text-center group">
                    <div className="bg-blue-100 dark:bg-blue-900 w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-4 group-hover:bg-blue-600 transition-colors duration-300">
                      <stat.icon className="w-8 h-8 text-blue-600 dark:text-blue-400 group-hover:text-white" />
                    </div>
              ...
              <div className="space-y-6">
                ...
                <div className="space-y-4">
                  <div className="flex items-start space-x-3">
                    ...
                    <div>
                      <h4 className="font-semibold text-gray-900 dark:text-white">{t('about.heritage.family')}</h4>
                      <p className="text-gray-600 dark:text-gray-300">{t('about.heritage.familyDesc')}</p>
                    </div>
                  </div>
                  <div className="flex items-start space-x-3">
                    <CheckCircle className="w-6 h-6 text-blue-600 mt-1 flex-shrink-0" />
                    <div>
                      <h4 className="font-semibold text-gray-900 dark:text-white">{t('about.heritage.growth')}</h4>
                      <p className="text-gray-600 dark:text-gray-300">{t('about.heritage.growthDesc')}</p>
                    </div>
                  </div>
                  <div className="flex items-start space-x-3">
                    <CheckCircle className="w-6 h-6 text-blue-600 mt-1 flex-shrink-0" />
                    <div>
                      <h4 className="font-semibold text-gray-900 dark:text-white">{t('about.heritage.leadership')}</h4>
                      <p className="text-gray-600 dark:text-gray-300">{t('about.heritage.leadershipDesc')}</p>
                    </div>
                  </div>
                </div>
              </div>
            ...
            {values.map((value, index) => (
              <Parallax key={index} speed={index % 2 === 0 ? -2 : 2}>
                <div className="card p-8 group hover:scale-105 transition-all duration-300">
                  <div className="bg-blue-100 dark:bg-blue-900 w-16 h-16 rounded-full flex items-center justify-center mb-6 group-hover:bg-blue-600 transition-colors">
                    <value.icon className="w-8 h-8 text-blue-600 dark:text-blue-400 group-hover:text-white" />
                  </div>
